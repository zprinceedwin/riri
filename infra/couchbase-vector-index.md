...
In Capella > Settings > Database Access, create a user with read/write on the `Riri` bucket. Note the username/password — these go in `.env` as `COUCHBASE_USERNAME` / `COUCHBASE_PASSWORD`.
...
- Name: `riri_vector_idx` (must match `COUCHBASE_VECTOR_INDEX` in `.env`)
- Bucket: `Riri`
- Scope: `_default`
...
```bash
pnpm --filter @riri/api exec tsx -e "
